...

{
  ...
  SQL parameter usage:
   ...
   - Added new type TSQLiteQuery (this is just a simple record wrapper around the SQL ansiString and a TSQLiteStmt pointer)
   ...
   - Added overloaded BindSQL methods for Integer and ansiString types - these set new values for the prepared query parameters
...
begin
  ...
  sql := 'select [sql] from sqlite_master where [type] = ''table'' and lower(name) = ''' +
    lowercase(TableName) + ''' ';
  ds:=  TSQLiteTable.Create(Self, SQL)  ;
    ...
    begin
      ...
      case pInteger(self.fColTypes[iColNo])^ of
        ...
        dtStr:
          ...
          begin
            setstring(ansiString(fResults[i]^), nil, 0);
            dispose(fResults[i]);
          end;
      else
        dispose(fResults[i]);
