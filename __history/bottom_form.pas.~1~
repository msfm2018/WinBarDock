...
begin
  ...
  if LVexeinfo.Items.Count = 0 then //无文件
  begin
...
begin
  ...
  if LVexeinfo.SelCount > 0 then
    if MessageBox(handle, '确认', '删除', MB_ICONQUESTION + MB_YESNO) <> IDYes then
      Exit;
  ...
  begin
    ...
    g_core.db.desktopdb.DeleteValue(FilePath);
    Node.Delete;
  end;
...
begin
  ...
  var va := g_core.db.desktopdb.GetString(ExeName);
  if (va <> '') then
    exit;

  FileName := ChangeFileName(FileName);
  g_core.db.desktopdb.SetVarValue(FileName, Path);

  ShowIOO(Path, FileName); //显示图标
end;
...
begin
  ...
  var keys := g_core.db.desktopdb.GetKeys;
  for var key in keys do

    ShowIOO(g_core.db.desktopdb.GetString(key), key); //显示图标
...
begin
  ...
  begin
    ...
    begin
      ...
      bmpIco.PixelFormat := pf24bit;    //保存图标
